...
object DateUtil {
  ...
  val sdf2=FastDateFormat.getInstance("yyyy-MM-dd")
  val sdf3=FastDateFormat.getInstance("yyyy/MM/dd")
  ...
  def addMonth1(date: Date, numMonth: Integer): Timestamp = {
    val cal = Calendar.getInstance();
    cal.setTime(date)
    cal.add(Calendar.MONTH, numMonth)
  ...
  /**
    ...
    * @param date
    */
  def etlFormat3(date:Date): String ={
    sdf3.format(date)
  }
  ...
  def getLastDay(date:Date,date_type:String="yyyy-MM-dd"):String = {
    val cal = Calendar.getInstance()
    cal.setTime(date)
    cal.set(Calendar.DAY_OF_MONTH,cal.getActualMaximum(Calendar.DAY_OF_MONTH))
    FastDateFormat.getInstance(date_type).format(cal.getTime)
  }


  def getLastDay(date:Date):Date = {
    val cal = Calendar.getInstance()
    cal.setTime(date)
    cal.set(Calendar.DAY_OF_MONTH,cal.getActualMaximum(Calendar.DAY_OF_MONTH))
    cal.getTime
  ...
  def main(args: Array[String]): Unit = {
    val yesterday = DateUtil.parse("20191231")
